...
def encrypt_message(message, mode='ECB', format='bytes', debug=False):
  aes = AESCipher()
  iv = token_bytes(16)
  ...
  if mode == 'ECB':
    ...
    ct = aes.ECBencrypt(message, output_format=format)
    ...
    ct = aes.CBCencrypt(message, iv, output_format=format)
    ...
    ct = aes.PBCencrypt(message, iv, output_format=format)
...
class AESCipher:
  ...
  def ECBdecrypt(self, ciphertext, input_format='bytes'):
    plaintext = self.cipher.decrypt(self.convert(ciphertext, input_format, 'bytes'))
    return plaintext
