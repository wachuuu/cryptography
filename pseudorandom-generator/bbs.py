...
class BBS:
  ...
  def __init__(self, p=None, q=None, seed=None) -> None:
    (self.__p, self.__q) = (p, q) if (p and q) else self.__generate_blum_numbers()
    self.__n = self.__q * self.__p
    self.seed = seed if seed else self.__generate_seed()

  def __str__(self) -> str:
    return f'p: {self.__p}, q: {self.__q}, seed: {self.seed}'
  ...
  def __generate_seed(self) -> int:
    seed_candidate = randint(1, self.__n - 1)
    while gcd(seed_candidate, self.__n) != 1:
      seed_candidate = randint(1, self.__n - 1)
    return seed_candidate

  def generate_bits(self, length, show_properties=False, filepath=None) -> str or None:
    next_val = pow(self.seed, 2, self.__n) # x0 = (seed^2) % n
    bits = str(next_val % 2)
    for _ in range(1, length):
      next_val = pow(next_val, 2, self.__n)
      bits += str(next_val % 2)
    if filepath:
      fp = open(filepath, 'w')
      if show_properties: fp.write(f'{self.__p};{self.__q};{self.seed}\n')
      fp.write(bits)
      fp.close()
    else:
      return bits
